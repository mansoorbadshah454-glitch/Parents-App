...
const ParentPortal = () => {
    ...
    const [notifications, setNotifications] = useState([
        { id: 1, type: 'attendance', student: 'Ali Ahmed', status: 'absent', time: '10:00 AM', read: false },
        { id: 2, type: 'score', student: 'Zainab Ahmed', subject: 'Math', score: '96%', time: 'Yesterday', read: true }
    ]);
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                                        <div style={{ display: 'flex', gap: '10px' }}>
                                            ...
                                            <div>
                                                ...
                                                <p style={{ fontSize: '0.8rem', color: 'var(--fb-muted)' }}>{post.timestamp ? new Date(post.timestamp.toDate()).toLocaleDateString() : 'Just now'} • <Users size={12} /></p>
                                            </div>
                                        </div>
...
const NotificationsView = ({ items, t }) => (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ padding: '15px' }}>
        ...
        {items.map(item => (
            <div key={item.id} className="card" style={{ padding: '15px', display: 'flex', gap: '15px', background: item.read ? 'transparent' : 'rgba(24, 119, 242, 0.05)' }}>
                <div style={{
                    width: '50px', height: '50px', borderRadius: '50%',
                    background: item.type === 'attendance' ? '#FEF2F2' : '#E7F3FF',
                    display: 'flex', alignItems: 'center', justifyContent: 'center'
                }}>
                    {item.type === 'attendance' ? <BellRing color="#dc2626" /> : <Award color="var(--fb-blue)" />}
                </div>
                <div style={{ flex: 1 }}>
                    <p style={{ fontSize: '0.95rem' }}>
                        {item.type === 'attendance' ? (
                            <><b>{item.student}</b> is <span style={{ color: '#dc2626', fontWeight: '700' }}>ABSENT</span> today.</>
                        ) : (
                            <><b>{item.student}</b> scored {item.score} in {item.subject}!</>
                        )}
                    </p>
                    <p style={{ fontSize: '0.8rem', color: 'var(--fb-muted)', marginTop: '4px' }}>{item.time}</p>
                </div>
            </div>
        ))}
    </motion.div>
